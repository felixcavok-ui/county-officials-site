...
// 同级单位不一定是市；历史数据的通用类型可用明确的名称后缀校正显示。
function unitType(c) {
  if (c.level !== 'prefecture') return c.type || '';
  if (/自治州$/.test(c.name || '')) return '自治州';
  if (/盟$/.test(c.name || '')) return '盟';
  if (/地区$/.test(c.name || '')) return '地区';
  if (/市$/.test(c.name || '')) return '地级市';
  return c.type || '地级单位';
}

function postTitles(c) {
  if (c.level === 'province') return { sec: '中共省委书记', mayor: '省长', secShort: '书记', mayorShort: '省长' };
  if (c.level === 'prefecture') {
    const type = unitType(c);
    if (type === '自治州') return { sec: '中共州委书记', mayor: '州长', secShort: '州委书记', mayorShort: '州长' };
    if (type === '盟') return { sec: '中共盟委书记', mayor: '盟长', secShort: '盟委书记', mayorShort: '盟长' };
    if (type === '地区') return { sec: '中共地委书记', mayor: '行署专员', secShort: '地委书记', mayorShort: '行署专员' };
    if (type === '地级市' || /市$/.test(c.name || '')) return { sec: '中共市委书记', mayor: '市长', secShort: '书记', mayorShort: '市长' };
    return { sec: '党委书记', mayor: '行政正职', secShort: '党委书记', mayorShort: '行政正职' };
  }
...
function ymToFloat(ym, boundary = 'start') {
  ...
  if (month) {
    ...
    if (monthNo >= 1 && monthNo <= 12) {
      const days = new Date(Number(month[1]), monthNo, 0).getDate();
      const day = month[3] ? Number(month[3]) : 1;
      if (day >= 1 && day <= days) return Number(month[1]) + (monthNo - 1 + (day - 1) / days) / 12;
    }
  }
  return DB.meta.yearMin; // 非法值由发布闸拦截；前端保持有限坐标
}
function pct(ym, boundary = 'start') {
  const axisEnd = Math.max(DB.meta.yearMax + 0.5, ymToFloat(DB.meta.released));
  const span = axisEnd - DB.meta.yearMin;
...
// 只有完整边界可以画任期长度。缺一端仅定位已知端点，点观测仅定位证据锚点。
function timelinePoint(s) { return Boolean(s.point || !s.start || (!s.end && s.exit !== 'incumbent')); }
function barStart(s) { return s.point ? (s.startX || s.endX) : (s.start || s.end); }
function barEnd(s) { return timelinePoint(s) ? barStart(s) : (s.exit === 'incumbent' ? DB.meta.released : s.end); }
...
function renderHeatmap() {
  ...
  hm.addEventListener('mousemove', e => {
    ...
    const label = agg ? '已记录端点覆盖率（成员均值）' : '书记+行政正职 已记录端点月度覆盖率';
...
function cardHTML(k) {
  ...
  if (k.level === 'prefecture') {
    ...
    return `<div class="unit-card" data-go="${escHTML(hash)}"><div class="uc-name">${escHTML(k.name)}</div>
      <div class="uc-meta">${escHTML(unitType(k))}</div><div class="uc-rec">下辖区县</div></div>`;
  }
...
function renderChildGrid(loc) {
  ...
  if (loc.level === 'nation') {
    ...
    kids = prefsOf(loc.prov); title = provName(loc.prov) + ' · 下辖 ' + kids.length + ' 个地级单位';
  } else if (loc.level === 'prefecture') {
...
function displayLane(spells) {
  const out = [];
  const monthOrDay = value => {
    const match = /^(\d{4})-(0[1-9]|1[0-2])(?:-(0[1-9]|[12]\d|3[01]))?$/.exec(String(value || ''));
    return Boolean(match && (!match[3] || Number(match[3]) <= new Date(Number(match[1]), Number(match[2]), 0).getDate()));
  };
  for (const s of spells) {
    ...
    if (prev && !prev.gap && !s.gap && !prev.point && !s.point && prev.pid && s.pid &&
        prev.pid === s.pid && prev.acting && !s.acting && prev.start && prev.end && s.start &&
        monthOrDay(prev.end) && monthOrDay(s.start) &&
        (prev.end.startsWith(s.start) || s.start.startsWith(prev.end)) &&
        ymToFloat(prev.start) <= ymToFloat(s.start)) {
      out[out.length - 1] = {
        ...s,
        start: prev.start,
        startX: prev.start,
...
function laneHTML(spells, title, en, dissolved) {
  ...
  const unpositionedPoints = displayed.filter(s => !s.gap && !barStart(s)).length;
  let bars = displayed.map((s, i) => {
    // 没有合格“在任年点”时间锚的薄记录只在下方任职表展示；时间轴不能伪装到当前年。
    if (!s.gap && !barStart(s)) return '';
    if (s.gap) {
      const gl = Math.max(pct(s.start, 'start'), 0);
      const gw = Math.min(pct(s.end, 'end'), 100) - gl;
      if (gw <= 0 || gl > 100) return '';
    ...
    const point = timelinePoint(s);
    const rawLeft = pct(barStart(s), 'start');
    const rawRight = pct(barEnd(s), 'end');
    if (rawLeft > 100 || rawRight < 0 || rawRight < rawLeft) return '';
    const clippedL = rawLeft < 0;                  // 起点早于观察窗 → 钳制到左缘并加 ← 标记
    const left = Math.max(rawLeft, 0);
    const width = Math.max(Math.min(rawRight, 100) - left, 0);
    ...
    const boundaryNote = point && !s.point
      ? `<br>仅定位已知${s.start ? '到任' : '离任'}时间，${s.start ? '离任' : '到任'}时间待查；标记不表示任期长度`
      : '';
    const identityNote = s.identityWarning ? '<br>同名人物关联待核，请分别核对各地记录' : '';
    const tip = (s.point
      ...
      离任去向：${escHTML(exitLabel)}${s.next && s.next !== '—' ? ' · ' + escHTML(s.next) : ''}${actingNote}${boundaryNote}`) + identityNote;
    ...
    const ariaLabel = `${s.name}，${s.point ? fmtStart(s) + (s.currentObservation ? ' 现任观察' : ' 在任年点') : fmtStart(s) + '至' + fmtEnd(s)}${
      pid ? '' : '，人物档案待建'}`;
    return `<div class="tl-bar exit-${Object.hasOwn(EXIT_LABEL, s.exit) ? s.exit : 'unknown'}${narrow ? ' is-narrow' : ''}${point ? ' is-point' : ''}"
      style="left:${left}%;width:${width}%;animation-delay:${i * 60}ms${pid ? '' : ';cursor:help'}"
      ${personAttrs} data-tip="${escHTML(tip)}"
      aria-label="${escHTML(ariaLabel)}">${point ? '' : (clippedL ? '← ' : '') + escHTML(s.name) + (pid ? '' : '（档案待建）')}</div>`;
  }).join('');
...
function epStatusHTML(status) {
  if (status === 'multi') return '<span class="mono verify-2" title="挂接材料来自多个独立机构；仍须核对材料是否支持该端点与日期精度">多源</span>';
  if (status === 'single') return '<span class="mono verify-1" title="挂接材料来自一个独立机构；来源数量不等于结论已核实">单源</span>';
...
function spellRows(spells, post) {
  return spells.filter(s => !s.gap).map(s => {
    const pid = String(s.pid || '').trim();
    const identityWarning = s.identityWarning || Boolean(DB.persons[pid] && DB.persons[pid].identityWarning);
    const identityTag = identityWarning ? '<span class="identity-tag">身份待核</span>' : '';
    const personHint = '<span class="dim" title="person_id 缺失，暂无人物档案"> · 档案待建</span>';
    const personCell = (content, rowspan = '') => pid
      ? `<td${rowspan} class="td-person" data-pid="${escHTML(pid)}">${content}${identityTag}</td>`
    ...
    <tr class="sp-row sp-arrival">
      ...
      <td class="td-detail"><span class="exit-tag" style="background:${identityWarning ? ORIGIN_COLOR.unknown : ORIGIN_COLOR[s.originType] || '#9a948a'}">${identityWarning ? '身份待核' : escHTML(s.origin || '待查')}</span>${!identityWarning && s.originPrior ? `<div class="next-line" title="${escHTML(s.originPrior)}">原职：${escHTML(s.originPrior.length > 26 ? s.originPrior.slice(0, 25) + '…' : s.originPrior)}</div>` : ''}</td>
    ...
    <tr class="sp-row sp-departure">
      ...
      <td class="td-src"><span class="dim">本版标为在任，暂无离任记录</span></td>
...
function unitPanelHTML(c, showBadge) {
  ...
  return `
    ...
    <div class="county-meta">
      <span class="chip">${escHTML(unitType(c))}</span>
    ...
    <div class="tl-legend">
      ...
      <span class="legend-note">◆ 仅有时间锚点，不表示任期长度；起止相接的代理期并入正式任期条（详见下表）</span>
    </div>
    ...
    <p class="table-note">每条任职段的<strong>到任</strong>与<strong>离任</strong>各自独立取证、分行列出。缺一端仅在图上定位已知端点；来源数量不代表材料已证明具体日期。至今以本版数据日期 ${escHTML(DB.meta.released)} 为界。</p>
    <p class="origin-legend">到任来源方向：
      <span class="exit-tag" style="background:var(--seal)">本级提拔</span>
      <span class="exit-tag" style="background:var(--ochre)">外地平调</span>
  ...
  for (const uid in DB.spells) {
    ...
    ['sec', 'mayor'].forEach(k => {
      DB.spells[uid][k].forEach(s => {
        if (s.pid === pid && !s.gap && c) out.push({ ...s, uid, county: c.name, post: k === 'sec' ? postTitles(c).secShort : postTitles(c).mayorShort });
      });
    });
  }
  return out;
...
function metaCell(label, val, conflicts) {
  if (Array.isArray(conflicts) && conflicts.length) {
    return `<div class="pm-cell"><dt>${escHTML(label)}</dt><dd class="conflicted">资料冲突待核
      <details class="field-conflict"><summary>查看各版本</summary>${conflicts.map(x => `<div>${escHTML(x)}</div>`).join('')}</details></dd></div>`;
  }
...
function renderPerson(pid) {
  ...
  const conflicts = p.fieldConflicts || {};
  const identityWarning = Boolean(p.identityWarning);
  const fieldLabels = { birth: '出生年月', gender: '性别', native: '籍贯', ethnic: '民族', edu: '最高学历', party: '入党时间', work: '参加工作' };
  const identityHTML = identityWarning ? `<div class="data-warning" role="note">
    <strong>同名人物关联待核</strong>
    <p>这个编号关联的各地资料存在多项身份冲突，尚未确认属于同一个人。合并属性、履历和纪律记录暂停展示；下表保留各地任职记录，可进入相应单位核对证据。</p>
    <details><summary>查看冲突字段（各版本的身份归属待核）</summary><dl class="person-meta">
      ${Object.entries(conflicts).map(([key, values]) => metaCell(fieldLabels[key] || key, '', values)).join('')}
    </dl></details></div>` : '';
  const careerConflictNote = p.careerConflicts ? `<div class="data-warning" role="note">有 ${escHTML(p.careerConflicts)} 组履历编号冲突，相关经历暂未展示，资料冲突待核。</div>` : '';

  const careerHTML = p.career && p.career.length ? p.career.map(s => `
    <div class="career-item ${s.county ? 'is-county' : ''}">
      <div class="ci-dates">${s.start ? escHTML(fmt(s.start)) : '？'}<br>– ${s.end ? escHTML(fmt(s.end)) : '？'}</div>
  ...
  const rosterHTML = roster.length ? `
    <h3 style="font-family:var(--serif);margin:1.8rem 0 .6rem">${identityWarning ? '关联的各地任职记录（身份待核）' : '本库任职记录'}</h3>
    <table class="data-table"><thead>
      <tr><th>单位</th><th>职位</th><th>任期</th><th>离任去向</th><th>核验</th></tr></thead><tbody>
      ${roster.map(r => `<tr>
        <td><a href="#/browse/${encodeURIComponent(r.uid)}">${escHTML(r.county)}</a></td><td>${escHTML(r.post)}${r.acting ? '（代）' : ''}</td>
  ...
  const purgeHTML = p.purge && !identityWarning ? `
    <div class="purge-box">
      <h4>纪律审查与司法记录</h4>
      <dl>
  ...
  $('#person-panel').innerHTML = `
    ...
    <div class="person-head">
      ...
      <span class="chip">${identityWarning ? '身份待核' : escHTML(p.status)}</span>
    </div>
    ${identityHTML}
    ${identityWarning ? '' : `<dl class="person-meta">
      ${metaCell('出生年月', p.birth, conflicts.birth)}
      ${metaCell('籍贯', p.native, conflicts.native)}
      ${metaCell('民族', p.ethnic, conflicts.ethnic)}
      ${metaCell('最高学历', p.edu, conflicts.edu)}
      ${metaCell('入党时间', p.party, conflicts.party)}
      ${metaCell('参加工作', p.work, conflicts.work)}
    </dl>`}
    ${purgeHTML}
    ${identityWarning ? '' : `<div class="career">
      <h3>履历（career spells）</h3>
      ${careerConflictNote}
      ${careerHTML}
    </div>`}
    ${rosterHTML}
    <p class="principle-line">※ ${identityWarning ? '关联来源（身份归属待核）：' : escHTML(p.srcNote || '')}${p.src ? srcChipsHTML(p.src) : ''}　<button class="btn-flag" data-flag="${escHTML(`${p.name}|人物档案|${pid}`)}">报告本页错误</button></p>
...
document.addEventListener('mousemove', e => {
  ...
  if (t) {
    showTip(`<div class="tt-title">${t.dataset.tip.split('<br>')[0]}</div>
             <div class="tt-sub">${t.dataset.tip.split('<br>').slice(1).join('<br>')}</div>`, e.clientX, e.clientY);
    return;
  }
...
// 全局搜索包含市级本级及区县，精确命中优先，方便直接核对市级记录。
function searchUnits(query) {
  const q = String(query || '').trim().toLowerCase();
  if (!q) return [];
  return DB.counties.filter(c => String(c.name || '').toLowerCase().includes(q) || String(c.code || '').includes(q))
    .sort((a, b) => Number(String(b.name || '').toLowerCase() === q) - Number(String(a.name || '').toLowerCase() === q)
      || Number(b.level === 'prefecture') - Number(a.level === 'prefecture'))
    .slice(0, 12);
}
$('#unit-search-input').addEventListener('input', e => {
  renderSearchResults(searchUnits(e.target.value));
});
